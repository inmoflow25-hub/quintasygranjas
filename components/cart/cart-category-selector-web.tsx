...
export default function CartCategorySelectorWeb() {
  ...
  useEffect(() => {
    async function loadProducts() {
      try {
        ...
        console.error("Error cargando productos web:", error)
      } finally {
  ...
  useEffect(() => {
    try {
      const savedCart = localStorage.getItem("qyg_checkout_cart")
      ...
      console.error("Error leyendo carrito web:", error)
    }
  }, [])
  ...
  useEffect(() => {
    try {
      ...
      console.error("Error guardando carrito web:", error)
    }
  }, [cart])
  ...
  function handleCategoryChange(category: string) {
    ...
    window.requestAnimationFrame(() => {
      document
        .getElementById("productos-listado-web")
        ?.scrollIntoView({
          behavior: "smooth",
          block: "start"
        })
  ...
  function handleCheckout() {
    if (cart.length === 0) {
      document
        .getElementById("selector-categorias-web")
        ?.scrollIntoView({
          behavior: "smooth",
          block: "start"
        })
                ...
                <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
                  {visibleProducts.map((product) => {
                    ...
                    return (
                      ...
                      >
                        <div className="aspect-[1.2/1] overflow-hidden bg-[#e7e1d8]">
                          {product.image ? (
                            <img
                              src={product.image}
                              alt={product.name}
